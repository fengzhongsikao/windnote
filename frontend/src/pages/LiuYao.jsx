...
function generateGua(method, input) {
  ...
  if (method === 'random') {
    ...
    for (let i = 0; i < 6; i++) {
      const bit = (lower >> i) & 1
      lines.push(bit === 1 ? 3 : 2)
    }
    const idx = moving === 0 ? 5 : moving - 1
    lines[idx] = lines[idx] === 3 ? 1 : 0
  }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-2 mb-6">
        <Hexagon size={24} className="text-qing-400" />
      ...
      <Card className="bg-bai-400 border-hei-400/10 mb-6">
        <CardBody className="space-y-4">
          ...
          >
            ...
            >
              <p className="text-sm text-hei-400/60 mt-2">
                系统将使用随机数生成卦象，适合快速占卜。
              </p>
            </Tab>
            <Tab
          ...
          <Card className="bg-bai-400 border-hei-400/10">
            <CardBody>
              ...
              {aiLoading ? (
                ...
                <div className="whitespace-pre-wrap text-hei-400/70 leading-relaxed typewriter-cursor">
                  {aiResponse}
                </div>
              )}
            </CardBody>
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="3xl">
        <ModalContent>
          <ModalHeader className="text-hei-400">爻辞详解</ModalHeader>
          <ModalBody>
            {selectedYao && (
